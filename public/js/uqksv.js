...
function getMembers() {
	...
	$.ajax({
		dataType: "json",
		url: "/getListMemberDelegate?psid="+document.getElementById("psid").value,
		data: objMember,
		success: function (data) {
			objMember = data;
...
function drawTable(objMembers) {
	...
	for (var i = 0; i < objMembers.length; i++) {
		...
		var strRow='<tr>';
			if(obj.BlockStatus=='ACTIVE' && (obj.Delegate==0 || obj.Delegate==null))
			{
				strRow= strRow+ ' <td scope="col" width="120px" height="120px" style="text-align: center;align:center"><img src="'+obj.ImgUrl+'" alt="Ảnh đại diện" height="80px" width="80px"><br><br><input type="image" src="img/uqksvon.png" alt="Hủy" onClick="AccpetMember(this);return false;" height="20px" width="80px"/></td>';
			}else if(obj.BlockStatus=='ACTIVE' && obj.Delegate==1)
			{
				strRow= strRow+ ' <td scope="col" width="120px" height="120px" style="text-align: center;align:center"><img src="'+obj.ImgUrl+'" alt="Ảnh đại diện" height="80px" width="80px"><br><br><input type="image" src="img/uqksvoff.png" alt="Hủy" onClick="CancelMember(this);return false;" height="20px" width="80px"/></td>';		
			}else
			{
...
function AccpetMember(objImg) {
	...
	//objM.ApprovedName=objMember[index].Name;
	 var mess = "Bạn có muốn uỷ quyền KSV cho " + objMember[index].Name + ", chức vụ " +objMember[index].Position +" ở " + local + ", có số ĐT:"+objM.Phone;
	var r = confirm(mess);
    if (r == true) {
		$.ajax({
			type: 'POST',
			...
			success: function(data) 
			{
				...
				row.cells[0].innerHTML='<td scope="col" width="120px" height="120px" style="text-align: center;align:center"><img src="'+objMember[index].ImgUrl+'" alt="Ảnh đại diện" height="80px" width="80px"><br><br><input type="image" src="img/uqksvoff.png" alt="Hủy" onClick="CancelMember(this);return false;" height="21px" width="80px"/></td>';
			},
			error: function(err) {		 
...
function CancelMember(objImg) {
	...
	objM.Name=objMember[index].Name;
	objM.ApprovedId=document.getElementById("psid").value;
	 var mess = "Bạn có muốn huỷ uỷ quyền KSV của " + objMember[index].Name + ", chức vụ " +objMember[index].Position +" ở " + local + ", có số ĐT:"+objM.Phone;
	var r = confirm(mess);
    if (r == true) {
	$.ajax({
		type: 'POST',
		data: JSON.stringify(objM),
		contentType: 'application/json',
		url: '/cancelDelegateMember.bot',				
		success: function(data) 
		{
			alert(data);			
			console.log(data);
			row.cells[0].innerHTML='<td scope="col" width="120px" height="120px" style="text-align: center;align:center"><img src="'+objMember[index].ImgUrl+'" alt="Ảnh đại diện" height="80px" width="80px"><br><br><input type="image" src="img/uqksvon.png" alt="Hủy" onClick="AccpetMember(this);return false;" height="21px" width="80px"/></td>';
		},
	    error: function(err) {		 
		  alert(err.statusText);
